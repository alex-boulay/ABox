...
public:
    ...
    VkCommandBuffer  getCommandBuffer(uint32_t index) { return commandBuffers.at(index); }

    VkCommandBuffer *getCommandBufferPtr(uint32_t index) { return &commandBuffers.at(index); }
};
...
public:
    CommandsHandler(VkDevice                                       device,
