...
DEFINE_VK_MEMORY_WRAPPER(VkPipelineLayout, PipelineLayout, vkDestroyPipelineLayout)
...
public:
    ...
    GraphicsPipeline(const SwapchainManager                      &sm,
                     VkDevice                                     device,
                     std::vector<VkPipelineShaderStageCreateInfo> shaderStages);

    ~GraphicsPipeline() = default;

    [[nodiscard]] VkPipeline        getPipeline() const noexcept { return graphicsPipeline; }
    ...
    [[nodiscard]] const VkViewport *getViewportPtr() const noexcept { return &viewport; }

    [[nodiscard]] VkRect2D          getScissor() const noexcept { return scissor; }

    [[nodiscard]] const VkRect2D   *getScissorPtr() const noexcept { return &scissor; }
