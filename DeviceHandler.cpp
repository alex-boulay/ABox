...
QueueFamilyIndices DeviceHandler::loadNecessaryQueueFamilies(uint32_t phyDev, VkSurfaceKHR surface)
{
...
VkResult DeviceHandler::addLogicalDevice(uint32_t index, VkSurfaceKHR surface)
{
    ...
    std::vector<const char *> devExtVect(deviceExtensions.begin(), deviceExtensions.end());
    listQueueFamilies();
...
VkResult DeviceHandler::addSwapchain(uint32_t width, uint32_t height, VkSurfaceKHR *surface, uint_fast8_t devIndex)
{
    ...
    std::cout << "DBE logical " << (void *) devices.at(devIndex) << '\n';
    std::cout << "DBE surface " << (void *) surface << '\n';
    std::cout << "DBE rQDI " << std::boolalpha << deviceMap.at(devIndex).fIndices.presentQueueIndex.has_value() << '\n';
    std::cout << "DBE gQDI " << std::boolalpha << deviceMap.at(devIndex).fIndices.graphicQueueIndex.has_value() << '\n';

    deviceMap.at(devIndex).swapchain.emplace(deviceMap.at(devIndex).physical,
                                             surface,
                                             devices.at(devIndex),
...
std::pair<VkResult, VkShaderModule> DeviceHandler::loadShader(uint_fast16_t deviceIndex, const ShaderDataFile &sdf)
{
    ...
    if (deviceMap.contains(deviceIndex))
    {
        if (!deviceMap.at(deviceIndex).loadedShaders.contains(sdf.getName()))
        {
            ...
            result                       = vkCreateShaderModule(devices.at(deviceIndex), &sdm, nullptr, &sm);
            if (result != VK_SUCCESS)
            {
            ...
            else
            {
                deviceMap.at(deviceIndex)
                    .loadedShaders.emplace(sdf.getName(), ShaderModuleWrapper(devices.at(deviceIndex), sm));
...
VkResult DeviceHandler::addGraphicsPipeline(uint32_t deviceIndex, const std::list<ShaderDataFile> &shaderFiles)
{
    ...
    if (deviceMap.contains(deviceIndex) && deviceMap.at(deviceIndex).swapchain.has_value())
    {
        std::cout << "Loading Graphics Pipeline " << std::endl;
        deviceMap.at(deviceIndex).graphicsppl = GraphicsPipeline(deviceMap.at(deviceIndex).swapchain.value(),
