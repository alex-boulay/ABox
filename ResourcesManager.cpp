...
VkResult ResourcesManager::addLogicalDevice(uint32_t physicalDeviceIndex)
{
...
{
    return devices.hasDevice(devIndex) ? devices.addSwapchain(width, height, &this->surface, devIndex) :
                                         VK_ERROR_DEVICE_LOST;
}

VkResult ResourcesManager::addGraphicsPipeline(std::vector<ShaderDataFile> smcis, uint32_t devIndex)
{
    std::cout << " devIndex : " << devIndex << " - device exists  " << devices.hasDevice(devIndex) << std::endl;
    std::cout << "ShaderDataFiles size : " << smcis.size() << std::endl;
    return devices.hasDevice(devIndex) ? devices.addGraphicsPipeline(devIndex, smcis) : VK_ERROR_DEVICE_LOST;
}
