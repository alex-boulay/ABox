    ...
    : extent(ext)
{
    ...
    window = glfwCreateWindow(ext.width, ext.height, title.c_str(), nullptr, nullptr);
    if (!window)
    {
...
void WindowManager::framebufferResizeCallback([[maybe_unused]] GLFWwindow *window, int width, int height)
{
    WindowManager *wm = static_cast<WindowManager *>(glfwGetWindowUserPointer(window));
    wm->extent        = VkExtent2D{.width = static_cast<uint32_t>(width), .height = static_cast<uint32_t>(height)};
...
{
    VkResult res = glfwCreateWindowSurface(rm.getInstance(), window, nullptr, rm.getWindowSurfacePtr());
    if (res != VK_SUCCESS)
    {
        std::stringstream ss;
        std::cout << "getInstance value " << rm.getInstance() << std::endl;
        ss << "Failed to create Vulkan surface! res value : " << (int32_t) res << std::endl;
        throw std::runtime_error(ss.str().c_str());
    }
    return res;
}

VkResult WindowManager::createSwapchain(ResourcesManager &rm, uint_fast8_t devIndex) const
{
