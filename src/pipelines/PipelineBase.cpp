...
void PipelineBase::buildReflectionData(const std::vector<const ShaderDataFile*>& shaders) {
  ...
  for (const ShaderDataFile* shader : shaders) {
    ...
    for (uint32_t i = 0; i < reflectionData.descriptorSetCount; ++i) {
      ...
      FILE_DEBUG_PRINT("Processing descriptor set %u with %u bindings", set->set, set->binding_count);
      ...
      for (uint32_t j = 0; j < set->binding_count; ++j) {
        ...
        // Check if this binding already exists in this set from another shader stage
        auto& bindings = setBindingsMap[set->set];
        auto existingBinding = std::find_if(bindings.begin(), bindings.end(),
        ...
        if (existingBinding != bindings.end()) {
          ...
          FILE_DEBUG_PRINT("  Merged binding %u with existing (stages: %x)",
            layoutBinding.binding, existingBinding->stageFlags);
        } else {
          bindings.push_back(layoutBinding);
          FILE_DEBUG_PRINT("  Added binding %u, type: %d, count: %u",
            layoutBinding.binding, layoutBinding.descriptorType, layoutBinding.descriptorCount);
        }
      }
    }
    ...
    for (uint32_t i = 0; i < reflectionData.pushConstantCount; ++i) {
      ...
      auto existingRange = std::find_if(pushConstantRanges.begin(), pushConstantRanges.end(),
      ...
      if (existingRange != pushConstantRanges.end()) {
        ...
        FILE_DEBUG_PRINT("Merged push constant range (offset: %u, size: %u, stages: %x)",
          range.offset, range.size, existingRange->stageFlags);
      } else {
        pushConstantRanges.push_back(range);
        FILE_DEBUG_PRINT("Added push constant range (offset: %u, size: %u, stage: %x)",
          range.offset, range.size, range.stageFlags);
      }
    }
  }
  ...
  for (const auto& [setIndex, bindings] : setBindingsMap) {
    ...
    FILE_DEBUG_PRINT("Created descriptor set layout for set %u with %zu bindings",
      setIndex, bindings.size());
  }
}
...
void PipelineBase::createPipelineLayout() {
  ...
  FILE_DEBUG_PRINT("Created pipeline layout with %zu descriptor sets and %zu push constant ranges",
    layouts.size(), pushConstantRanges.size());
}
...
void PipelineBase::printReflectionInfo() const {
  ...
  std::cout << "Push Constant Ranges: " << pushConstantRanges.size() << std::endl;
  ...
  for (size_t i = 0; i < pushConstantRanges.size(); ++i) {
    ...
    std::cout << "  Push Constant " << i << ": offset=" << range.offset
              << ", size=" << range.size
              << ", stages=0x" << std::hex << range.stageFlags << std::dec << std::endl;
  }
}
