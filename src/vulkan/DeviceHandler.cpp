...
std::set<VkQueueFlags> necessaryDeviceQueueFamilyFLags{VK_QUEUE_GRAPHICS_BIT, VK_QUEUE_COMPUTE_BIT};
...
bool supportsPresentation(VkPhysicalDevice pD, VkSurfaceKHR surface, uint32_t qFamIndex)
{
    VkBool32 support;
    VkResult result = vkGetPhysicalDeviceSurfaceSupportKHR(pD, qFamIndex, surface, &support);
    if (result != VK_SUCCESS)
    {
        std::cerr << "Error Querying Physical Device Support for KHR Surfaces !\n";
        std::cerr << "Phy " << pD << " - Surface" << surface << " - qFamIndex " << qFamIndex << std::endl;
...
uint32_t DeviceHandler::listQueueFamilies()
{
    ...
    for (uint16_t i = 0; i < phyDevices.size(); i++)
    {
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(phyDevices.at(i), &queueCount, nullptr);
        std::vector<VkQueueFamilyProperties> queueFamilies(queueCount);
        vkGetPhysicalDeviceQueueFamilyProperties(phyDevices.at(i), &queueCount, queueFamilies.data());
        for (uint16_t qi = 0; qi < queueFamilies.size(); qi++)
        {
            std::cout << "\t QueueFamily n°" << qi << '\n';
            std::cout << queueFamilies.at(qi) << '\n';
            std::cout << "Is a valid QueueFamily : " << std::boolalpha << bool(isValidQueueFamily(queueFamilies.at(qi)))
                      << '\n';
        }
    }
    std::cout << std::endl;
...
VkResult DeviceHandler::DeviceExtensionSupport(VkPhysicalDevice device)
{
    ...
    {
#ifdef DEBUG_VK_ABOX
        std::cout << "Dev extension : " << extension.extensionName << " - Version : " << extension.specVersion << '\n';
#endif
...
std::unordered_map<QueueRole, uint32_t> DeviceHandler::loadNecessaryQueueFamilies(uint32_t phyDev, VkSurfaceKHR surface)
{
    ...
    std::vector<VkQueueFamilyProperties> queueFamilies(queueCount);
    vkGetPhysicalDeviceQueueFamilyProperties(pPD, &queueCount, queueFamilies.data());
...
std::vector<uint32_t> DeviceHandler::listQueueFamilyIndices(QueueFamilyIndices fi)
{
    auto s = getQueueFamilyIndices(fi);
    return std::vector<uint32_t>(s.cbegin(), s.cend());
}

VkResult DeviceHandler::addLogicalDevice(VkSurfaceKHR surface, std::string name, uint32_t index)
{
    if (surface == VK_NULL_HANDLE)
    {
        throw std::runtime_error("Given Surface is not initialised (or has already been freed).");
    }
    const float                             queuePriority = 1.0f;

    VkPhysicalDevice                        phydev   = phyDevices[index];
    std::unordered_map<QueueRole, uint32_t> fIndices = loadNecessaryQueueFamilies(index, surface);
    ...
    {
        qCI.push_back(VkDeviceQueueCreateInfo{.sType            = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
                                              .pNext            = nullptr,
                                              .flags            = 0u,
    ...
    if (res == VK_SUCCESS)
    {
        ...
        if (deviceNames.contains(name))
        {
            std::cout << "Overlaping the device name : " << name
                      << " - Device : " << (void *) deviceNames.at(name)->getDevice() << " Might not have been freed"
                      << std::endl;
        }
    ...
    vkGetDeviceQueue(dev, fIndices.at(QueueRole::Graphics), 0, &devices.back().graphicsQueue);

    vkGetDeviceQueue(dev, fIndices.at(QueueRole::Present), 0u, &(devices.back().presentQueue));

    return res;
}    // namespace ABox_Utils

uint32_t rateDeviceSuitability(VkPhysicalDeviceProperties deviceProperties, VkPhysicalDeviceFeatures deviceFeatures)
{
    uint32_t score = 0;
...
uint32_t DeviceHandler::findBestPhysicalDevice()
{
    ...
    {
#ifdef DEBUG_VK_ABOX
        std::cout << "Physical Device selected is PhyDevice n°" << candidates.rbegin()->second << "\n";
#endif
        return candidates.rbegin()->second;
    }
    else
    {
        throw std::runtime_error("None of the installed GPU are compatible with the Application ! \n");
    }
    return 0;
}
...
VkResult DeviceHandler::addSwapchain(uint32_t width, uint32_t height, VkSurfaceKHR *surface, uint_fast8_t devIndex)
{
    ...
    std::cout << "DBE rQDI " << std::boolalpha << devicePtr->getFamilyQueueIndices().at(QueueRole::Present) << '\n';
    std::cout << "DBE gQDI " << std::boolalpha << devicePtr->getFamilyQueueIndices().at(QueueRole::Graphics) << '\n';

    getDBE(devIndex)->swapchain.emplace(devicePtr->getPhysicalDevice(),
                                        surface,
                                        getDevice(devIndex),
...
std::pair<VkResult, VkShaderModule> DeviceHandler::loadShader(uint_fast16_t deviceIndex, const ShaderDataFile &sdf)
{
    VkShaderModule       sm        = {0};
    ...
    if (devicePtr)
    {
        if (!devicePtr->loadedShaders.contains(sdf.getName()))
        {
            ...
            if (result != VK_SUCCESS)
            {
                std::cout << "error allocating the shader Module \n\t VK_ERROR CODE : " << result << std::endl;
            }
            else
            {
                devicePtr->loadedShaders.emplace(std::piecewise_construct,
    ...
    {
        std::cout << "Error : Device couldn't be found !! check device Index " << std::endl;
    }
    return {result, sm};
}

VkResult DeviceHandler::addGraphicsPipeline(uint32_t deviceIndex, const std::list<ShaderDataFile> &shaderFiles)
{
    ...
    if (dbe && dbe->swapchain.has_value())
    {
        ...
        dbe->graphicsppl.emplace(dbe->swapchain.value(), getDevice(deviceIndex), PSSCIs);
        return VK_SUCCESS;
    }
    std::cout << "Failed to initialise Graphics Pipeline in the Device Manager" << std::endl;
...
std::stringstream vkQueueFlagSS(const VkQueueFlags &flag)
{
    ...
    ss << std::boolalpha << "\nVK_QUEUE_GRAPHICS_BIT - 0x1 : " << bool(flag & VK_QUEUE_GRAPHICS_BIT)
       ...
       << "\nVK_QUEUE_SPARSE_BINDING_BIT - 0x8: " << bool(flag & VK_QUEUE_SPARSE_BINDING_BIT)
       << "\nVK_QUEUE_PROTECTED_BIT - 0x10: " << bool(flag & VK_QUEUE_PROTECTED_BIT)
       << "\nVK_QUEUE_VIDEO_DECODE_BIT_KHR - 0x20: " << bool(flag & VK_QUEUE_VIDEO_DECODE_BIT_KHR)
       << "\nVK_QUEUE_VIDEO_ENCODE_BIT_KHR - 0x40: " << bool(flag & VK_QUEUE_VIDEO_ENCODE_BIT_KHR)
       << "\nVK_QUEUE_OPTICAL_FLOW_BIT_NV - 0x100: " << bool(flag & VK_QUEUE_OPTICAL_FLOW_BIT_NV)
       << "\nVK_QUEUE_FLAG_BITS_MAX_ENUM - 0x7FFFFFFF: " << bool(flag & VK_QUEUE_FLAG_BITS_MAX_ENUM);
    return ss;
}

OSTREAM_OP(const VkExtent3D &ext)
{
    os << " Width : " << ext.width << " - Height : " << ext.height << " - Depth : " << ext.depth;
    return os;
}
