...
[[nodiscard]] ExtensionFileResult readExtentions(std::filesystem::path path)
{
    ...
    ExtensionFileResult result{.status = VK_FILE_EXTENTION_ERROR, .platform = SourcePlatform::Unknown, .stage = nullptr};
...
inline ShaderDataFile::operator VkShaderModuleCreateInfo() const
{
    return {.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
            .pNext = nullptr,
            .flags = std::get<VkShaderStageFlagBits>(*stage),    // maybe more than one in the future ? to overload with
                                                                 // Shader creation.
            .codeSize = code.size(),
            .pCode    = code.data()};
}
...
const std::string ShaderHandler::loadShaderFromFile(const std::filesystem::path &shaderFile)
{
    ...
    if (!file.is_open())
    {
        throw std::runtime_error(std::string("Couldn't load Shader File") + shaderFile.string());
    }
    return std::string((std::istreambuf_iterator<char>(file)), (std::istreambuf_iterator<char>()));
}

const std::vector<uint32_t> ShaderHandler::compileGLSLToSPIRV(const std::string &shaderCode,
...
{
    return std::accumulate(sDatas.begin(),
                           sDatas.end(),
                           std::string(),
                           [](const std::string &a, const ShaderDataFile &b) { return a + b.getName() + '\n'; });
}

ShaderDataFile *ShaderHandler::getShader(std::string name)
{
    std::vector<ShaderDataFile>::iterator result = std::find_if(sDatas.begin(),
                                                                sDatas.end(),
                                                                [&name](const ShaderDataFile &a) -> bool
                                                                { return a.getName() == name; });
    return result != sDatas.end() ? &(*result) : nullptr;
}

[[nodiscard]] ShaderHandler::operator std::vector<VkShaderModuleCreateInfo>() const
{
