...
const std::map<std::string, SourcePlatform> extensionToPlatform = {
    {".glsl", SourcePlatform::GLSL}, {".hlsl", SourcePlatform::HLSL},
    {".fx", SourcePlatform::HLSL},   {".cl", SourcePlatform::OpenCL},
    {".cu", SourcePlatform::CUDA},   {".wgsl", SourcePlatform::WGSL},
    {".rs", SourcePlatform::Rust},   {".py", SourcePlatform::Python}};
...
getEShSource(SourcePlatform sourcePlatform) {
  ...
  case (SourcePlatform::GLSL):
    return glslang::EShSourceGlsl;
  case (SourcePlatform::HLSL):
...
[[nodiscard]] ExtensionFileResult readExtentions(std::filesystem::path path) {
  ...
  ExtensionFileResult result{.status = VK_FILE_EXTENTION_ERROR,
  ...
  if (!extensions.size() ||
    ...
    FILE_DEBUG_PRINT("File ignored due to unsuported extension : %s",
...
VkResult ShaderDataFile::load(const VkDevice *&device) {
  if (sBinds.end() !=
      std::find_if(sBinds.begin(), sBinds.end(),
                   [device](shaderBind a) { return a.device == device; })) {
  ...
  VkShaderModuleCreateInfo createInfo{
      ...
      .pNext = nullptr,
      .flags = std::get<VkShaderStageFlagBits>(
          *stage), // maybe more than one in the future ? to overload with
                   // Shader creation.
      .codeSize = code.size(),
      .pCode = code.data()};
  ...
  FILE_DEBUG_PRINT("Creating shader module from file %s to device %p",
                   name.c_str(), device);
  VkResult result =
...
ShaderHandler::loadShaderDataFile(const std::filesystem::path &filePath) {
  if (std::filesystem::exists(filePath)) {
    ...
    if (extFileResult.status == VK_FILE_SUCCESS) {
      ...
      sDatas.push_back(ShaderDataFile(filePath.filename(), code,
                                      extFileResult.stage,
...
ShaderHandler::loadShaderFromFile(const std::filesystem::path &shaderFile) {
  ...
  if (!file.is_open())
    throw std::runtime_error(std::string("Couldn't load Shader File") +
                             shaderFile.string());
...
ShaderHandler::compileGLSLToSPIRV(const std::string &shaderCode,
  ...
  shader.setEnvInput(glslang::EShSourceGlsl, shaderStage,
                     glslang::EShClientVulkan, OPENGL_CHOSEN_VERSION);
...
std::string ShaderHandler::listAllShaders() {
  return std::accumulate(sDatas.begin(), sDatas.end(), std::string(),
...
ShaderDataFile *ShaderHandler::getShader(std::string name) {
  std::vector<ShaderDataFile>::iterator result = std::find_if(
      sDatas.begin(), sDatas.end(),
...
uint32_t ShaderHandler::loadAllShaders(const VkDevice *device) {
  return std::accumulate(sDatas.begin(), sDatas.end(), 0u,
                         [&](uint32_t acc, ShaderDataFile &s) -> uint32_t {
                           return acc + (s.load(device) == VK_SUCCESS);
                         });
}
