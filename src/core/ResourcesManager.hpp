...
public:
    ...
    ABox_Utils::DeviceBoundElements  *getDevice(uint32_t device) { return deviceHandler->getDBE(0u); }
    ...
    void drawFrame(uint32_t devIndex = 0u)
    {
        ...
        VkSubmitInfo         submitInfo{
                    ...
                    .commandBufferCount = static_cast<uint32_t>(    // TODO maybe not size but binding to a
                                                            // specific one
                dbe->getCommandHandler()->top().commandBuffers.size()),
            .pCommandBuffers =
                dbe->getCommandHandler()->top().commandBuffers.data(),    // TODO after bound to queue management -
                    .signalSemaphoreCount = 1,
                    .pSignalSemaphores    = dbe->getFrameSyncArray()->getFrameSyncObject()->renderEnd.ptr(),
        };
        VkResult result = vkQueueSubmit(dbe->graphicsQueue,
                                        1,
                                        &submitInfo,
