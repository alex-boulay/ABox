...
public:
    ...
    {
        return std::find_if(map.begin(), map.end(), [key](const auto &v) { return key == std::get<T>(v); });
    }

    template<typename T> inline static constexpr bool contains(T key) { return map.end() != at(key); }

    [[nodiscard]] inline std::optional<EShLanguage>   getStageExt(std::string stageExt) noexcept
    {
...
public:
    ...
    ~ShaderDataFile() = default;

    ShaderDataFile(const ShaderDataFile &other)                = default;
    ShaderDataFile &operator=(const ShaderDataFile &other)     = delete;    // stage cannot be modified
    ShaderDataFile(ShaderDataFile &&other) noexcept            = default;
    ShaderDataFile &operator=(ShaderDataFile &&other) noexcept = delete;    // stage connot be modified

    inline          operator VkShaderModuleCreateInfo() const
    {
        return {.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
                .pNext = nullptr,
                .flags = std::get<VkShaderStageFlagBits>(*stage),    // maybe more than one in the future ? to overload
    ...
    [[nodiscard]] inline VkPipelineShaderStageCreateInfo getPSSCI(VkShaderModule shm) const
    {
...
class ShaderHandler
{
     ...
     */
    inline bool                 initGlsLang() { return isGlsInit ? true : (isGlsInit = glslang::InitializeProcess()); }
...
public:
    ...
    inline const std::vector<ShaderDataFile> getShaderHandlers() const { return sDatas; }
};
