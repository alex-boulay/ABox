...
public:
    ...
    std::pair<VkResult, VkShaderModule> loadShader(uint_fast16_t deviceIndex, ShaderDataFile sdf);

    /**
     * @brief add a GraphicsPipeline to a LogicalDevice which must have a
     * swapchain
     */
    VkResult addGraphicsPipeline(uint32_t deviceIndex, std::vector<ShaderDataFile> shaderFiles);
