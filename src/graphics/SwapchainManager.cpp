...
SwapchainManager::SwapchainManager(VkPhysicalDevice phyDev,
    ...
    , queueFamilyIndices(rQDI, gQDI)
{
    ...
    if (formatCount != 0)
    {
        ...
        vkGetPhysicalDeviceSurfaceFormatsKHR(phyDev, *surface, &formatCount, formats.data());
    }
    ...
    if (presentModeCount != 0)
    {
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(phyDev, *surface, &presentModeCount, presentModes.data());
    }

    if (formats.empty() || presentModes.empty())
    {
        std::stringstream ss;
        ss << std::boolalpha << "Could not create swapchain :\n\t Formats : " << (!formats.empty())
...
VkResult SwapchainManager::createSwapchain(VkPhysicalDevice phyDev, VkDevice logicalDevice)
{
    ...
    VkSwapchainCreateInfoKHR createInfo{.sType                 = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
                                        ...
                                        .queueFamilyIndexCount = scscConcurent *
                                                                 static_cast<uint32_t>(queueFamilyIndices.size()),
                                        .pQueueFamilyIndices = scscConcurent ? queueFamilyIndices.data() : nullptr,
    ...
    VkResult                 return_value = vkCreateSwapchainKHR(logicalDevice, &createInfo, nullptr, &newSwapchain);
...
VkResult SwapchainManager::createImageViews(VkDevice device)
{
    ...
    for (const auto &img : _images)
    {
        ...
        {
            std::cout << "Image Views created - n°" << swapChainImages.size() + 1 << std::endl;
        }
...
VkResult SwapchainManager::createFramebuffers(VkRenderPass renderPass, VkDevice logicalDevice)
{
    uint32_t i = 0u;
    std::cout << "Creating FrameBuffers - for size " << swapChainImages.size() << std::endl;
    for (auto &a : swapChainImages)
    {
        ...
        VkResult creation_return_value = vkCreateFramebuffer(logicalDevice, &fbi, nullptr, &_swapchainFramebuffer);
...
VkResult SwapchainManager::resizeSwapChain(VkPhysicalDevice phyDev, VkDevice device, VkRenderPass rp)
{
    framebuffers.clear();
...
VkResult SwapchainManager::resizeSwapChain(VkPhysicalDevice phyDev, VkDevice device, VkExtent2D window, VkRenderPass rp)
{
    extent = window;
