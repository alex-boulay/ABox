...
ResourcesManager::ResourcesManager()
{
    ...
    VkDebugUtilsMessengerCreateInfoEXT debugCreateInfo = debugHandler.populateDebugMessenger();
    ...
    std::cout << "after create instance info " << &instanceCreateInfo << std::endl;
    VkResult res = vkCreateInstance(&instanceCreateInfo, nullptr, instance.ptr());
    if (res != VK_SUCCESS)
    {
        std::stringstream ss;
        ss << "Resources Manager Error : failed to create instance! VkResult = " << res << std::endl;
...
VkResult ResourcesManager::addLogicalDevice(uint32_t physicalDeviceIndex, std::string name)
{
    return deviceHandler.value().addLogicalDevice(getWindowSurface(), name, physicalDeviceIndex);
}

VkResult ResourcesManager::createSwapchain(uint32_t width, uint32_t height, uint32_t devIndex)
{
    return deviceHandler.value().hasDevice(devIndex) ?
               deviceHandler.value().addSwapchain(width, height, getWindowSurfacePtr(), devIndex) :
...
VkResult ResourcesManager::addGraphicsPipeline(const std::list<ShaderDataFile> &smcis, uint32_t devIndex)
{
    std::cout << " devIndex : " << devIndex << " Instance Loaded DeviceHandler ? " << deviceHandler.has_value()
              << " - device exists  " << deviceHandler.value().hasDevice(devIndex) << std::endl;
    std::cout << "ShaderDataFiles size : " << smcis.size() << std::endl;
    return deviceHandler.value().hasDevice(devIndex) ? deviceHandler.value().addGraphicsPipeline(devIndex, smcis) :
                                                       VK_ERROR_DEVICE_LOST;
}
...
VkResult ResourcesManager::createFramebuffers(uint32_t devIndex)
{
    ...
    {
        throw std::runtime_error("No DeviceHandler allocated - impossible to create a framebuffer !");
    }
}
...
void ResourcesManager::drawFrame()
{
    ...
    VkSubmitInfo         submitInfo{
        ...
        .pCommandBuffers = dbe->getCommandHandler()->top().getCommandBufferPtr(
            frameIndex),    // TODO after bound to queue management -
                .signalSemaphoreCount = 1,
                .pSignalSemaphores    = dbe->getFrameSyncArray()->getFrameSyncObject()->renderEnd.ptr(),
    };
    result = vkQueueSubmit(dbe->graphicsQueue, 1, &submitInfo, dbe->getFrameSyncArray()->getFrameSyncObject()->inFlight);
