...
public:
        ...
        , inFlight(dev)
    {
        ...
        if (result == VK_SUCCESS)
        {
            std::cout << "RenderEnd Semaphore added !" << (void *) renderEnd << std::endl;
        }
        else
        {
            throw std::runtime_error("Couldn't Create Semaphore !");
...
public:
    FrameSyncArray(VkDevice device, int32_t arraySize = INFLIGHT_NUMBER_OF_ELEMENTS)
    {
        constexpr int32_t maxi16 = static_cast<int32_t>(std::numeric_limits<int16_t>::max());
        if (arraySize != std::clamp(arraySize, 0, maxi16))
        {
            std::stringstream error_s;
            error_s << " Invalid size for Frame Synchronisation "
                    << "Array (FrameSyncArray) : " << arraySize << " - whistl min = 0 and max = " << maxi16
                    << std::endl;
    ...
    FrameSyncObject *getFrameSyncObject() { return getFrameSyncObject(frameIndex); }

    uint32_t         size() const { return static_cast<uint32_t>(framesSync.size()); }

    int16_t          incrementFrameIndex() { return (frameIndex = ((frameIndex + 1) % framesSync.size())); }
    ...
    {
        vkWaitForFences(device, 1, framesSync.at(frameIndex).inFlight.ptr(), VK_TRUE, time);
