...
public:
    ...
    uint32_t getRenderQueueDeviceIndice() const { return queueFamilyIndices.getRenderQueueDeviceIndice(); };

    uint32_t getGraphicsQueueDeviceIndice() const { return queueFamilyIndices.getGraphicsQueueDeviceIndice(); };
    ...
    VkResult        resizeSwapChain(VkPhysicalDevice phyDev, VkDevice device, VkRenderPass rp);

    VkResult        resizeSwapChain(VkPhysicalDevice phyDev, VkDevice device, VkExtent2D window, VkRenderPass rp);

    // TODO:
    // Latter implementation for windowcallback
    // windowManager::resize(SwapchainManager sm): VkResult
    VkResult        resizeSwapChain(VkPhysicalDevice phyDev, VkDevice device, VkExtent2D window);
