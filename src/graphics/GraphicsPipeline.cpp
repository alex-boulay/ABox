    ...
    , pipelineLayout(device)
{
    ...
    res = vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, pipelineLayout.ptr());
    if (res != VK_SUCCESS)
    {
    ...
    {
        std::cout << "\tStage : " << a.stage << " - entry point : \"" << std::string(a.pName) << "\"" << std::endl;
    }
    ...
    res = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, graphicsPipeline.ptr());
    if (res != VK_SUCCESS)
    {
        std::stringstream ss;
...
VkResult GraphicsPipeline::CreateRenderPass(const SwapchainManager &sm, VkDevice device)
{
    VkAttachmentDescription colorAttachment{.flags          = 0u,
                                            .format         = sm.getFormat(),
    ...
    VkResult                res = vkCreateRenderPass(device, &renderPassInfo, nullptr, renderPass.ptr());
